...
breakers = [
    ...
    },
    {
    ...
    },
     {
        ...
        'description': 'Единственный ледокол в мире для круглогодичной проводки судов по Северному морскому пути при толщине льда свыше 4 м. За счет обширной автоматизации может работать без вмешательства операторов.',
        'requirements': [9,10],
        'maximum_progress': 2,
        'cost': 200,
        'parameters': [
...
class IceBreaker:
    ...
    def start_building(self, gamestate):
        if gamestate.money >= self.cost and self.progress == 0 and all(res['completed'] for res in self.requirements):
            gamestate.money -= self.cost
            self.progress = 1
